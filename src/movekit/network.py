...
def network_time_graphlist(preprocessed_data, object_type = 'delaunay_object'):
    ...
    data_groups_time = timewise_dict(centroid_medoid_computation(extract_features(preprocessed_data)))
    ...
    for item in range(0,len(del_dict[object_type])):
        ...
        dists = dists_time[item+1].drop(['time', 'animal_id', 'x', 'y'], axis=1).to_dict()


        values = list(itertools.chain.from_iterable(pd.DataFrame(dists).values.tolist()))

        keys = list(itertools.product(list(dists_time[item+1]['animal_id']),list(dists.keys())))
